...
VALUES (
    ...
    500,
    3000,
    'Ngưng tụ hàn khí thành kiếm quang, bắn ra một lưỡi băng đánh mục tiêu, gây 120% ATK.',
...
VALUES (
    ...
    'item_bang_kiem',
    'bg_item_epic',
    'Hàn kiếm đúc từ băng tinh. Tăng 50 ATK và đi kèm Băng Liệt Trảm.',
...
WHERE equipment_template_id = 910013;
...
WHERE NOT EXISTS (
    SELECT 1
    FROM public.player_items AS existing
    WHERE existing.player_id = c.id
      AND existing.item_template_id = 910012
);
...
WHERE NOT EXISTS (
    ...
    FROM public.player_items AS existing
    WHERE existing.player_id = c.id
      AND existing.item_template_id = 910013
);
