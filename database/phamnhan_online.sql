...
--
-- Name: repo_schema_alignment_server_phase1_security_guards; Type: DO; Schema: -; Owner: -
--
...
BEGIN
    IF EXISTS (
        SELECT 1
        FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'game_configs')
    THEN
    ...
    IF EXISTS (
        SELECT 1
        FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'player_items')
    THEN
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_player_items_player_location_template ON public.player_items(player_id, location_type, item_template_id)';
    END IF;
END
$$;
