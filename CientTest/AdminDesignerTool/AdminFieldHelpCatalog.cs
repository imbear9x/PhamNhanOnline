...
internal static class AdminFieldHelpCatalog
{
    ...
    private static readonly IReadOnlyDictionary<string, string> SpecificHelp =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["skill_effects.effect_type"] = "Loại effect runtime của skill.\n`Damage`: gây sát thương.\n`Heal`: hồi HP.\n`ResourceReduce`: trừ tài nguyên theo `resource_type` như HP/MP/Stamina.\n`ResourceRestore`: hồi tài nguyên.\n`Shield`: tạo lá chắn hấp thụ damage.\n`Stun`: làm choáng theo `duration_ms`.\n`BuffStat`/`DebuffStat`: cộng hoặc trừ chỉ số theo `stat_type` và `value_type`.",
            ["skill_effects.target_scope"] = "Phạm vi áp effect trong runtime.\n`Primary`: tác động lên mục tiêu chính của skill. Với skill Self thì Primary cũng chính là bản thân caster.\n`Self`: luôn tác động lên caster.\n`Splash` và `All` hiện để dành cho phase AOE, chưa nên dùng nếu server chưa hỗ trợ mục tiêu vùng.",
            ["skill_effects.trigger_timing"] = "Thời điểm effect được áp.\n`OnCast`: áp ngay khi kết thúc thi triển/gồng.\n`OnHit`: áp ở thời điểm impact sau travel time.\n`OnExpire` hiện để dành cho phase sau khi có hệ buff/debuff lifecycle đầy đủ.",
            ["skill_effects.resource_type"] = "Loại tài nguyên bị tác động khi effect là `ResourceReduce` hoặc `ResourceRestore`.\nThường dùng `Mp` để hồi hoặc trừ mana, `Stamina` cho thể lực.\nNếu muốn gây damage hoặc hồi HP mục tiêu thì ưu tiên dùng trực tiếp `Damage` hoặc `Heal` cho dễ đọc.",
            ...
            ["skill_effects.value_type"] = "Cách hiểu giá trị của effect.\nVới `BuffStat`/`DebuffStat`: `Flat` cộng trừ thẳng, `Percent` hiểu theo phần trăm, `Ratio` hiểu theo hệ số 0..1.\nVới damage/heal/resource, phase hiện tại server ưu tiên đọc magnitude từ `formula_type`, `base_value`, `ratio_value` và `extra_value`.",
