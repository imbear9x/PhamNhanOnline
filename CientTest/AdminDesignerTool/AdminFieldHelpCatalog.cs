...
internal static class AdminFieldHelpCatalog
{
        ...
        AdminColumnBinding? binding)
    {
        ...
        if (binding?.EnumType is not null)
            lines.Add($"Cách nhập: chọn một giá trị trong enum `{binding.EnumType.Name}`.");
        else if (!string.IsNullOrWhiteSpace(binding?.LookupSql))
            lines.Add("Cách nhập: chọn một giá trị từ danh sách liên kết (FK), không nên nhớ và gõ tay ID.");
        else if (column.DataType == typeof(bool))
            lines.Add("Kiểu dữ liệu: Có/Không.");
        else if (column.DataType == typeof(DateTime))
            lines.Add("Kiểu dữ liệu: thời gian. Thường có thể giữ giá trị mặc định mà hệ thống gợi ý.");
        else if (column.DataType == typeof(string))
            lines.Add("Kiểu dữ liệu: văn bản ngắn.");
        else
    ...
    private static string GetGenericHint(string columnName)
    {
        return columnName switch
        {
            ...
            "description" => "Ý nghĩa: ghi chú nghiệp vụ hoặc mô tả nội dung. Có thể viết tự do để người thiết kế khác hiểu được mục đích của data.",
            "created_at" => "Ý nghĩa: thời điểm tạo bản ghi. Thường có thể để hệ thống/DB tự điền.",
            "item_template_id" => "Ý nghĩa: trỏ đến Item Template cha. Nếu danh sách trống, cần tạo Item Template phù hợp trước.",
            ...
            "skill_effect_id" => "Ý nghĩa: trỏ đến một effect cụ thể của skill.",
            "craft_recipe_id" => "Ý nghĩa: trỏ đến recipe chế tạo cha.",
            ...
            "table_id" => "Ý nghĩa: mã bảng random mà code/runtime sẽ gọi tới. Nên đặt có namespace rõ ràng. Ví dụ: `monster.drop.demo_slime`.",
            ...
            "chance_parts_per_million" => "Ý nghĩa: tỉ lệ theo thang 1.000.000. Ví dụ: `50000 = 5%`, `100000 = 10%`, `250000 = 25%`.",
            "stat_type" => "Ý nghĩa: loại stat bị ảnh hưởng. Ví dụ: `Hp`, `Attack`, `Speed`.",
            "value_type" => "Ý nghĩa: cách hiểu giá trị, ví dụ cộng thẳng, tỉ lệ, phần trăm.",
            "formula_type" => "Ý nghĩa: công thức tính effect của skill. Ví dụ: đánh theo AttackRatio hay Flat.",
            "target_type" => "Ý nghĩa: kiểu mục tiêu mà skill/effect tác động vào. Ví dụ: bản thân, một địch, vùng.",
            "consume_mode" => "Ý nghĩa: cách hệ thống tiêu hao nguyên liệu khi chế tạo/luyện đan.",
            "required_quantity" => "Ý nghĩa: số lượng vật phẩm cần dùng cho một lần. Ví dụ: `3`.",
            ...
            "recipe_book_item_template_id" => "Ý nghĩa: Item Template của sách/đan phương dùng để học công thức.",
            ...
            "success_rate" => "Ý nghĩa: tỉ lệ thành công cơ bản. Hãy xem đúng quy ước của bảng này trước khi nhập.",
            "mutation_rate" => "Ý nghĩa: tỉ lệ đột biến của recipe.",
            "mutation_rate_cap" => "Ý nghĩa: trần trên của tỉ lệ đột biến sau khi cộng bonus.",
            ...
            _ => "Hãy nhập giá trị đúng theo nghiệp vụ của bảng này. Nếu đây là trường liên kết thì ưu tiên chọn từ dropdown thay vì nhớ ID."
        };
    }
    ...
    private static readonly IReadOnlyDictionary<string, string> SpecificHelp =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["pill_recipe_templates.recipe_book_item_template_id"] = "Chọn Item Template của sách/đan phương dùng để học recipe.\nVí dụ: `sách_đan_hồi_linh`.\nĐiều kiện: Item Template đó nên có loại `PillRecipeBook`.",
            ["pill_recipe_templates.result_pill_item_template_id"] = "Chọn Item Template của viên đan đầu ra.\nVí dụ: `hồi_linh_đan`.",
            ["pill_recipe_templates.base_success_rate"] = "Tỉ lệ thành công cơ bản của đan phương.\nVí dụ: `0.65` nếu bạn đang dùng quy ước 0..1.\nNên giữ nhất quán trên toàn bộ bảng.",
            ["pill_recipe_templates.mutation_rate"] = "Tỉ lệ đột biến cơ bản của đan phương.\nVí dụ: `0.05` nếu bạn muốn 5%.\nNếu chưa dùng cơ chế đột biến, có thể để `0`.",
            ["pill_recipe_inputs.required_item_template_id"] = "Nguyên liệu cần cho đan phương.\nVí dụ: `huyết_sâm`, `linh_thảo`, `nước_linh`.\nPhải chọn từ Item Template đã tồn tại.",
            ["pill_recipe_inputs.required_quantity"] = "Số lượng nguyên liệu cần cho một lần luyện.\nVí dụ: `2`, `5`, `10`.",
            ["pill_recipe_inputs.is_optional"] = "Đánh dấu nếu đây là nguyên liệu tùy chọn.\nNguyên liệu tùy chọn thường dùng để tăng tỉ lệ thành công hoặc đột biến.",
            ["pill_recipe_inputs.success_rate_bonus"] = "Bonus thêm vào tỉ lệ thành công khi dùng nguyên liệu tùy chọn này.\nVí dụ: `0.05` để cộng thêm 5%.",
            ["pill_recipe_inputs.mutation_bonus_rate"] = "Bonus thêm vào tỉ lệ đột biến khi dùng nguyên liệu tùy chọn này.",
            ...
            ["pill_recipe_mastery_stages.success_rate_bonus"] = "Bonus tỉ lệ thành công nhận được khi đạt mốc thông thạo.\nVí dụ: `0.03` để cộng 3%.",
            ["game_random_tables.table_id"] = "Mã bảng random mà code/runtime sẽ gọi.\nVí dụ: `monster.drop.demo_slime`, `boss.drop.hac_ho`, `craft.bonus.weapon_common`.\nNên đặt ổn định, có namespace rõ ràng.",
            ["game_random_tables.mode"] = "Chế độ hoạt động của bảng random.\nHiện tại runtime mới hỗ trợ `Exclusive`, nghĩa là chọn một kết quả cuối cùng sau khi chuẩn hóa tổng tỉ lệ.",
            ...
            ["game_random_entries.chance_parts_per_million"] = "Tỉ lệ của entry theo thang 1.000.000.\nVí dụ: `50000 = 5%`, `250000 = 25%`.",
